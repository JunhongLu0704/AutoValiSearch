...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None, dropout_method='none', drop_rate=0):
    ...
    def forward(self, x):
        ...
        if self.dropout_method == 'bn_drop':
...
class ResNet_with_table(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.head_drop = args.head_drop
        self.backbone_drop = args.backbone_drop
        self.drop_rate = args.drop_rate
        if args.not_dropout:
            self.head_drop = False
            self.backbone_drop = "none"
            self.drop_rate = 0.0
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False):
        ...
        layers.append(block(self.inplanes, planes, stride, downsample, self.groups, self.base_width, previous_dilation,
                            norm_layer, dropout_method=self.backbone_drop, drop_rate=self.drop_rate))
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
            layers.append(block(self.inplanes, planes, groups=self.groups,
                                base_width=self.base_width, dilation=self.dilation,
                                norm_layer=norm_layer, dropout_method=self.backbone_drop, drop_rate=self.drop_rate))
    ...
    def _forward_impl(self, x):
        ...
        return x, flatten_features.detach()
